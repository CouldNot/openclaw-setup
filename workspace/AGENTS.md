...
For every explicit reminder, add an active-state item with `kind: reminder` and an exact `dueAt`. That single mutation atomically creates the Discord notification; do not create a separate cron job. Treat the reminder as successfully scheduled only when the tool returns `reminder_scheduled: true`. If it errors or that confirmation is absent, say plainly that scheduling failed—never claim it is set merely because an active-state record exists.
